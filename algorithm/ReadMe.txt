================================================================================
    MICROSOFT 基础类库 : USV 项目概述
===============================================================================

应用程序向导已为您创建了此 USV 应用程序。此应用程序不仅演示 Microsoft 基础类的基本使用方法，还可作为您编写应用程序的起点。

本文件概要介绍组成 USV 应用程序的每个文件的内容。
...
USV.h
    这是应用程序的主头文件。
    其中包括其他项目特定的标头（包括 Resource.h），并声明 USVApp 应用程序类。

USV.cpp
    这是包含应用程序类 USVApp 的主应用程序源文件。

USV.rc
    这是程序使用的所有 Microsoft Windows 资源的列表。它包括 RES 子目录中存储的图标、位图和光标。此文件可以直接在 Microsoft Visual C++ 中进行编辑。项目资源包含在 2052 中。

res\USV.ico
    这是用作应用程序图标的图标文件。此图标包括在主资源文件 USV.rc 中。
...
res\Toolbar.bmp
    此位图文件用于为工具栏创建平铺图像。
    初始工具栏和状态栏在 CMainFrame 类中构造。使用资源编辑器编辑此工具栏位图，并更新 USV.rc 中的 IDR_MAINFRAME TOOLBAR 数组以添加工具栏按钮。
/////////////////////////////////////////////////////////////////////////////
...
USVDoc.h、USVDoc.cpp - 文档
    这些文件包含 USVDoc 类。编辑这些文件以添加特殊文档数据并实现文件保存和加载（通过 USVDoc::Serialize）。
    该文档将包含以下字符串：
        文件扩展名：      USV
        文件类型 ID：        USV.Document
        主框架标题：  USV
        文档类型名称：       USV
        筛选器名称：         USV Files (*.USV)
        文件的新短名称：USV
        文件类型长名称：USV.Document

USVView.h、USVView.cpp - 文档视图
    这些文件包含 USVView 类。
    USVView 对象用于查看 USVDoc 对象。

res\USVDoc.ico
    这是一个图标文件，用作 USVDoc 类的 MDI 子窗口的图标。此图标包括在主资源文件 USV.rc 中。
...
StdAfx.h, StdAfx.cpp
    这些文件用于生成名为 USV.pch 的预编译头 (PCH) 文件和名为 StdAfx.obj 的预编译类型文件。
